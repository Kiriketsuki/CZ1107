...
BTNode *searchNode(BTNode *node, int key)
{
    ...
    if (curr_node->item == key) {
        return curr_node;
    }

    if (curr_node->left != NULL) {
        to_return = searchNode(curr_node->left, key);
    }

    // for if the left recursion finds the node
    if (to_return != NULL) {
        return to_return;
    }

    if (curr_node->right != NULL) {
        to_return = searchNode(curr_node->right, key);
    }

    // dont need an if statement here since there is no more recursion to be had
