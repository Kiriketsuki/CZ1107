...
void levelOrderTraversal(BSTNode *root)
{
    ...
    enqueue(&queueueue.head, &queueueue.tail, root);

    while(!isEmpty(queueueue.head)) {
        curr = dequeue(&queueueue.head, &queueueue.tail);
        printf("%d ", curr->item);
        if (curr->left != NULL) {
            enqueue(&queueueue.head, &queueueue.tail, curr->left);
        }

        if (curr->right != NULL) {
            enqueue(&queueueue.head, &queueueue.tail, curr->right);
        }
    }
}
