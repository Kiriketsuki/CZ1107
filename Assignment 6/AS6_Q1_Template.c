...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    if (Q1Ptr->hSize == 0 || HashSearch(*Q1Ptr, key) != NULL) { // no table or key alr inserted
        return 0;
    }

    hash_address = Hash(key, Q1Ptr->hSize);
    old_head = Q1Ptr->Table[hash_address].head;
    curr = malloc(sizeof(ListNode)); // create new node
    curr->key = key; // add key
    curr->pre = NULL; // new node will be head so pre == null
    curr->next = old_head; // point to old head
    if (old_head != NULL) {
        old_head->pre = curr;
    } 
    Q1Ptr->Table[hash_address].head = curr; // new_head = curr
    Q1Ptr->Table[hash_address].size++;
    Q1Ptr->nSize++;
    return 1;
}
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    if (prev == NULL) { // head
        if (next == NULL) { // only element
            Q1Ptr->Table[hash_address].head = NULL;
        } else {
            Q1Ptr->Table[hash_address].head = curr->next;
            Q1Ptr->Table[hash_address].head->pre = NULL;
        }
    } else {
        if (next == NULL) {
            prev->next = NULL;
